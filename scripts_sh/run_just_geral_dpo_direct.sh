#!/bin/bash
#SBATCH --job-name=Llama_DPO_Direct
#SBATCH --output=../output/logs/dpo_direct-%j.out
#SBATCH --error=../output/logs/dpo_direct-%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "--- Job DPO DIRETO Iniciado ---"
cd $SRC_DIR

apptainer exec --nv \
    --bind $PROJECT_ROOT:$PROJECT_ROOT \
    $SIF_IMAGE \
    python3 -u train_just_geral_dpo_direct.py

echo "--- Job DPO DIRETO Finalizado ---"
